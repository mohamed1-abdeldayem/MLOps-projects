...
from src.mlops_churn_api import settings
import pandas as pd
...
class ModelTrainer:
    ...
    def __init__(self):
        ...
        self.Processed_Path = (
            self.BASE_DIR / "data" / "processed" / "Telco-Customer-Churn-processed.csv"
        )

        self.Feature_Path = self.BASE_DIR / "models" / "feature_names.json"
    ...
    def pre_processing(
        self, processed_path=None, feature_path=None
    ) -> tuple[pd.DataFrame, pd.Series]:
        processed_path = processed_path or self.Processed_Path
        feature_path = feature_path or self.Feature_Path
        ...
        log_regression_pipe = Pipeline(
            [
                ...
                (
                    "log_regression",
                    LogisticRegression(
                        C=10.0, penalty="l2", solver="liblinear", max_iter=10000
                    ),
                ),
            ]
        )
    ...
    def evaluate_model(
        self, x_test: pd.DataFrame, y_test: pd.Series, model: GridSearchCV
    ) -> None:
